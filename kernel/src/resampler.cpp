...
bool interpolation(const uint8_t k, const float* signal_original, const smpn_t samples_original, float** _signal_interpolated, smpn_t* _samples_interpolated)
{
    ...
    // interpolation
    for (uint32_t orig_smp_ind = 0, intrp_smp_ind = 0; orig_smp_ind < samples_original; orig_smp_ind++)
    {
        ...
        const float x0 = orig_smp_ind;
        const float x1 = orig_smp_ind + k;
...
{
    freq_t M = freq_original;
    freq_t N = freq_target;
    ...
    if (!interpolation(M, signal_original, samples_original, &signal_interpolated, &samples_interpolated))
    {
        return false;
    }
    ...
    if (!decimation(N, signal_interpolated, samples_interpolated, &signal_decimated, &samples_decimated))
    {
